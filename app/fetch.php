...
if(isset($_POST['filter_state'], $_POST['filter_cast'],$_POST['filter_year'],$_POST['filter_status']) 
&& $_POST['filter_state'] != ''&& $_POST['filter_status'] != '' && $_POST['filter_cast'] != '' && $_POST['filter_year'] != ''){
    
$query .= 'WHERE state ="'.$_POST['filter_state'].'" AND caste_details = "'.$_POST['filter_cast'].'" 
AND year = "'.$_POST['filter_year'].'" OR student_status = "'.$_POST['filter_status'].'" ';
...
$trnumber2 = str_replace(" ","",$trnumber1);   
  ...
  $message_code=$decodejson->message_code;
   ...
   if(($full_name_db_cleaned==$api_fetched_name_cleaned) and ($account_exists==true) and ($success==true) and($status_code==200) and($message_code=="success")){
    ...
    $sub_array = array();
    $sub_array[] = '<input type="checkbox" class="delete_checkbox" name="row-check" value="'.$row["student_id"].'">';
    $sub_array[] = $i++;
