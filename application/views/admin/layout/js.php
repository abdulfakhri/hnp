 ...
 <!-- Calendar JavaScript -->
    ...
    $(document).ready(function($) {
	 ...
	 <!-- Chart Files -->
		...
		<!-- <script src="<?php echo base_url(); ?>optimum/plugins/bower_components/flot.tooltip/flot.tooltip.js"></script> -->
...
<script src="<?php echo base_url(); ?>optimum/js/jquery.PrintArea.js" type="text/JavaScript"></script>
   ...
   <script>
    $(document).ready(function() {
        $('#myTable').DataTable({"pageLength": 50,lengthMenu: [[ 10, 25, 50, 100 ,200, 500, -1 ],[ '10 rows', '25 rows', '50 rows', '100 rows', '200 rows','500 rows', 'Show all' ]]});
        $(document).ready(function() {
            var groupColumn = 1;
...
function yesnoCheck2(that) {
        if (that.value == "other") {
      // alert("check");
            // document.getElementById("ifYesTrNumber").style.display = "block";
            document.getElementById("tr_number").readOnly = false;
            document.getElementById("tr_number").value = '';
            document.getElementById("tr_number").required = true;
        } else {
            document.getElementById("tr_number").readOnly = true
            document.getElementById("tr_number").value = "Pending";
        }
    }
...
// approve Button
           ...
           $(document).on('click','.approve_btn',function() { 
                console.log("JS")
                  var ul_html = '';
                    $('#exampleModalCenter').find('#exampleModalCenterTitle').html("Approve The Student")

                    ul_html = ' <ul class="icheck-list">'

                    ul_html +='<li class="college">';
                    ul_html +='<input type="radio" class="check" id="approved_by_college" name="approved_by_college" data-radio="iradio_flat-green"> <label for="approved_by_college">Approved By College</label></li>';

                    ul_html +='<li class="site">';
                     ul_html +='<input type="radio" class="check" id="approved_by_our_site" name="approved_by_our_site" data-radio="iradio_flat-green"> <label for="approved_by_our_site">Approved By Our Site</label></li>';

                    ul_html +='<li class = "nsp">';
                    ul_html +='<input type="radio" class="check" id="approved_by_nsp" name="approved_by_nsp" data-radio="iradio_flat-green"><label for="approved_by_nsp">Approved By NSP</label></li></ul>';

                    $('#exampleModalCenter').find('.modal-body').find('#student_status_form').html(ul_html);
                    $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').html('Approve');
                     $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').removeClass().addClass('btn btn-block btn-success btn-rounded btn-lg')
...
// Pending Student
  // $(".pending_btn").click(function(){
                 $(document).on('click','.pending_btn',function() { 
                          var ul_html = '';
                            $('#exampleModalCenter').find('#exampleModalCenterTitle').html("Pending")

                            ul_html = ' <ul class="icheck-list">';

                            ul_html +='<li class="college">';
                            ul_html +='<input type="radio" class="check" id="pending_by_college" name="pending_by_college" data-radio="iradio_flat-green"> <label for="pending_by_college">Pending By College</label></li>';

                            ul_html +='<li class="site">';
                            ul_html +='<input type="radio" class="check" id="pending_by_our_site" name="pending_by_our_site" data-radio="iradio_flat-green"> <label for="pending_by_our_site">Pending By Our Site</label></li>';

                            ul_html +='<li class="nsp">';
                            ul_html +='<input type="radio" class="check" id="pending_by_nsp" name="pending_by_nsp" data-radio="iradio_flat-green"><label for="pending_by_nsp">Pending By NSP</label></li></ul>';

                            ul_html +='<div class="form-group">';
                            ul_html +='<label class="col-md-12">Remarks</label>';
                            ul_html +='<div class="col-md-12">';
                           ul_html +='<textarea name="status_reason" class="form-control" required rows="5"></textarea>';
                           ul_html +='</div></div>';

                            $('#exampleModalCenter').find('.modal-body').find('#student_status_form').html(ul_html);
                           $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').html('Pending');
                            $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').removeClass().addClass('btn btn-primary btn btn-block btn-rounded btn-lg btn-warning')
...
// Defect Student
                ...
                $(document).on('click','.defect_btn',function() { 
                            var ul_html = '';
                            $('#exampleModalCenter').find('#exampleModalCenterTitle').html("Form Defect")

                            ul_html = ' <ul class="icheck-list">'


                            ul_html +='<li class="college">';
                            ul_html +='<input type="radio" class="check" id="defect_by_college" name="defect_by_college" data-radio="iradio_flat-green"> <label for="defect_by_college">Form Defect By College</label></li>';


                            ul_html +='<li class="site">';
                            ul_html +='<input type="radio" class="check" id="defect_by_our_site" name="defect_by_our_site" data-radio="iradio_flat-green"> <label for="defect_by_our_site">Form Defect By Our Site</label></li>';


                            ul_html +='<li class="nsp">';
                            ul_html +='<input type="radio" class="check" id="defect_by_nsp" name="defect_by_nsp" data-radio="iradio_flat-green"><label for="defect_by_nsp">Form Defect By NSP</label></li></ul>';
                            ...
                            $('#exampleModalCenter').find('.modal-body').find('#student_status_form').html(ul_html);
                            $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').html('Defect');
                            $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').removeClass().addClass('btn btn-block btn-primary btn-rounded btn-lg')
...
// Reject Student
 // $(".reject_btn").click(function(){
            $(document).on('click','.reject_btn',function() { 
                var ul_html = '';
                $('#exampleModalCenter').find('#exampleModalCenterTitle').html("Form Reject");
                    ul_html = ' <ul class="icheck-list">'

                    ul_html +='<li class="college">';
                    ul_html +='<input type="radio" class="check" id="college_reject" name="college_reject" data-radio="iradio_flat-green"> <label for="college_reject">Form Reject By College</label></li>';

                    ul_html +='<li class="site">';
                    ul_html +='<input type="radio" class="check" id="reject_by_our_site" name="reject_by_our_site" data-radio="iradio_flat-green"> <label for="reject_by_our_site">Form Reject By Our SIte</label></li>';

                    ul_html +='<li class="nsp">';
                    ul_html +='<input type="radio" class="check" id="nsp_reject" name="nsp_reject" data-radio="iradio_flat-green"><label for="nsp_reject">Form Reject By NSP</label></li></ul>';
                   ...
                   ul_html +='</div></div>';
                $('#exampleModalCenter').find('.modal-body').find('#student_status_form').html(ul_html);
                $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').html('Reject');
                $('#exampleModalCenter').find('.modal-footer').find('#modal_success_btn').removeClass().addClass('btn btn-block btn-danger btn-rounded btn-lg');
                $('#exampleModalCenter').modal('show');
            });
});
...
function wireUpEvents() {
  ...
  */
  window.onbeforeunload = function() {
      if (!validNavigation) {
        console.log("onbeforeunload",validNavigation);
        alert("KARTHIK")
         endSession();
      }
  }
  ...
  $(document).bind('keypress', function(e) {
    if (e.keyCode == 116){
      validNavigation = true;
         console.log("keyCode",validNavigation);
    }
  });
  ...
  $("a").bind("click", function() {
    validNavigation = true;
     console.log("a click",validNavigation);
  });
 
  // Attach the event submit for all forms in the page
  $("form").bind("submit", function() {
    validNavigation = true;
     console.log("form SECTION",validNavigation);
  });
...
$(document).ready(function() {
    ...
    var checkAll_Message = false;
    var single_msg = false;
    var group_msg = false;
newChatNotification();
newgroupChatNotification();
hederNotfication();
...
setInterval(function(){ 
   ...
   hederNotfication();
	...
	function hederNotfication(){
		if(single_msg&&group_msg){
			$(".notificationBox_count").html('M');
		} else if(single_msg){
			$(".notificationBox_count").html('M');
		}else if(group_msg){
			$(".notificationBox_count").html('M');
		}else {
			$(".notificationBox_count").html('');
		}
		  $.ajax({url: "<?php echo base_url(); ?>auth/validateUser", success: function(response){
                if(response){
        ...
        function newChatNotification(){
            var usernames = '<?=htmlspecialchars($_COOKIE["usernames"])?>';
            if(!usernames)
            {
                return false;
            }
            ...
            {
                var dataString = "user_to=" + usernames;
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>chat_user/vpb_new_chat_header_notfication.php",
                    data: dataString, 
                    cache: false,
                    beforeSend: function() {},  
                    ...
                    {   
                       ...
                       if(data.length === 0){
                            $(".message-center").html('');
                            $(".notificationBox_count").html('')
                            $(".drop-title").html('<i class="fa fa-bell" aria-hidden="true"></i> All Caught Up!!!!');
                            $('.chatStatus.newalertNotfication').hide();
                            $('.fa-bell.my-float').removeClass('animated');
                            // if(checkAll_Message){
                            //      $(".notificationBox_count").html('M');
                            // }
                            single_msg=false;
                         ...
                         single_msg = true;
                         $('.chatStatus.newalertNotfication').css('display','inline-block');
                         $('.fa-bell.my-float').addClass('animated');
                        $(".drop-title").html("You Have "+Object.keys(data).length+' Notfication.' );
                        // $(".notificationBox_count").html(Object.keys(data).length);
                        $(".notificationBox_count").html('M');
                        checkAll_Message =true;
                         $(".message-center").html('');
                        $.each(data, function(index, item) {
                            $(".message-center").append(data[index][0]);   
                            $.each(data[index],function(index2,item2){
                                $(".message-center").find('.'+index).find('.mail-contnet').find('.mail-desc').html('<strong>'+Object.keys(data[index]).length+' new message</strong>');
                           });                           
                        }); 
                        }                       
                         //newChatNotification()
        ...
        function newgroupChatNotification(){
            ...
            { 
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>ChatPhp/new_notify.php",
                    data: { id: userID},
                    success: function(response) 
                    {   
                       var data = JSON.parse(response);
                       ...
                       var notfiyHtml =''
                       if(data.length === 0){
                            $(".groupChat-message-center").html('');
                            $(".notificationBox_count").html('')
                            $(".groupChat-drop-title").html('<i class="fa fa-bell" aria-hidden="true"></i> No Group Message.!!!!');
                            $('.chatStatus.groupChatButtonStatus').hide();
                            $('.fa-bell.my-float').removeClass('animated');
                            group_msg = false;
                            // if(checkAll_Message){
                            //      $(".notificationBox_count").html('M');
                         ...
                         group_msg = true;
                         $('.chatStatus.groupChatButtonStatus').css('display','inline-block');
                         $('.fa-bell.my-float').addClass('animated');
                        $(".groupChat-drop-title").html("You have "+Object.keys(data).length+' group message.' );
                        $(".notificationBox_count").html('M');
                        checkAll_Message =true;
                         $(".groupChat-message-center").html('');
                        $.each(data, function(index, item) {
                            $(".groupChat-message-center").append(data[index][0]);   
                            $.each(data[index],function(index2,item2){
                               $(".groupChat-message-center").find('.'+index).find('.mail-contnet').find('.mail-desc').find('.message_count').html(Object.keys(data[index]).length+' messages in');
                           });                           
                        }); 
                        }                       
